...
#endif
{
  ...
  while( (client_sock = accept(s, NULL, 0) ) ) {
    ...
    if( pthread_create( &new_thread->thread , &attr ,  connection_handler , (void*) &new_thread->client_sock) < 0) {
      fprintf(stdout, "pthread_create failed, rc=%lx\n", trv);
      return (-1);
    }
#elif defeined(_THREADX_)
