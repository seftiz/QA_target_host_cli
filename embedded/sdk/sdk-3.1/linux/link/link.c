  ...
  for ( i = 2 ; i < argc; i += 2 ) {
    ...
    if ( strlen(str_data) ) {
      if ( strcmp( (char*) str_data,  "data") == 0 ) {
        ...
        link_sk_attr.proto.frame_type = NAV_SE_CSV_TCP_IP4;
      } 
      else {
        link_sk_attr.proto.frame_type = V2X_FRAME_DATA;
      } 
...
int cli_v2x_link_tx( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )
{
  ...
  if ( !strlen(tx_data) ) {
		...
		if (payload_len > 0 ) {
			...
			sprintf(tx_data, "Some test");
		}
  }
  msg_size = strlen(tx_data) + 1;
  //cli_print( cli, "DBG : tx data %s, size %d", tx_data, (int) msg_size );
...
int cli_v2x_link_rx( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )
{
  ...
  while ( frames-- ) {
    ...
    if (msg[v2x_buf_size(&buf) - 1] != '\0') {
			cli_print( cli,"ERROR : Bad message (not zero-terminated)\n");
      goto error;
    }
