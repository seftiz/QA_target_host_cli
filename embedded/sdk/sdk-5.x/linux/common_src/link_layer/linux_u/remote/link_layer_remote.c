...
#define LL_MAX_DEVICE_COUNT 4
#define LL_INTERFACE_NAME_MAX 40

static struct {
  int context;
  char interface_name[LL_INTERFACE_NAME_MAX];
} ll_context_array[LL_MAX_DEVICE_COUNT] = {{0}};

static int ll_context_index = 0;

static rdev_desc_t
ll_remote_device[LL_MAX_DEVICE_COUNT] = {
  {
  .rdev_address.octets = { 0x00, 0x02, 0xcc, 0xf0, 0x00, 0x07},
  .rdev_interface_name = NULL,
  .link_ops_ptr = &ll_ops,
  .link_context_ptr = &ll_context_array[0].context,
  .hw_ops_ptr = NULL,
  .hw_context_ptr = NULL,
  },
  {
  ...
  .rdev_interface_name = NULL,
  .link_ops_ptr = &ll_ops,
  .link_context_ptr = &ll_context_array[1].context,
  .hw_ops_ptr = NULL,
  .hw_context_ptr = NULL,
  },
  {
  ...
  .rdev_interface_name = NULL,
  .link_ops_ptr = &ll_ops,
  .link_context_ptr = &ll_context_array[2].context,
  .hw_ops_ptr = NULL,
  .hw_context_ptr = NULL,
  },
  {
  ...
  .rdev_interface_name = NULL,
  .link_ops_ptr = &ll_ops,
  .link_context_ptr = &ll_context_array[3].context,
  .hw_ops_ptr = NULL,
  .hw_context_ptr = NULL,
  },
};
...
ll_get(const char *dev_name)
{
  ...
  if (dev_name == NULL) {
    return &ll_remote_device[0];
  }

  for (index = 0; index < LL_MAX_DEVICE_COUNT; index++) {
    if (strcmp(ll_context_array[index].interface_name,
               dev_name) == 0) {
      return &ll_remote_device[index];
    }
  }
...
ll_init(const char *dev_name)
{
  ...
  if (ll_context_index >= LL_MAX_DEVICE_COUNT) {
    fprintf(stderr, "No space for additional device in link layer\n");
    return -1;
  }

  memcpy(eth_dev_name, dev_name, LL_DEV_NAME_LEN);
  strncpy(ll_context_array[ll_context_index].interface_name,
          dev_name,
          sizeof(ll_context_array[ll_context_index].interface_name));
  ...
  ll_context_array[ll_context_index].context = socket_fd;
  ll_context_index++;
