...
int cli_v2x_ecc_service_create( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc ) 
{
  ...
  if ( strcmp( (char*) str_data,  "hw") == 0 ) {
	...
	/* Get ECC HW service */
		rc = ecc_hw_service_get(&ecc_service);
		if (atlk_error(rc)) {
...
int cli_v2x_ecc_curve_type_get( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  CHECK_NUM_ARGS /* make sure all parameter are there */
	...
	if ( IS_SIGN || IS_ALL ) {
    cli_print ( cli, "ECC sign curve type : %d" , myctx->ecc_info.signing_request.curve );
	}
  
  if ( IS_VERIFY || IS_ALL ) {
  	cli_print ( cli, "ECC verify curve type : %d" ,  myctx->ecc_info.verification_request.curve );
  }
...
int cli_v2x_ecc_curve_type_set( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  CHECK_NUM_ARGS /* make sure all parameter are there */
	...
	if ( IS_SIGN || IS_ALL ) {
		myctx->ecc_info.signing_request.curve = user_curve;
	}
 	if ( IS_VERIFY || IS_ALL ) {
    myctx->ecc_info.verification_request.curve = user_curve;
  }
...
int cli_v2x_ecc_socket_create( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc ) 
{
  ...
  CHECK_NUM_ARGS /* make sure all parameter are there */
    ...
    myctx->ecc_info.signing_socket = NULL;
		rc = ecc_signing_socket_create(ecc_service, &myctx->ecc_info.signing_socket);
		if (atlk_error(rc)) {
    ...
    myctx->ecc_info.verification_socket = NULL;
		rc = ecc_verification_socket_create(ecc_service, &myctx->ecc_info.verification_socket);
		if (atlk_error(rc)) {
...
int cli_v2x_ecc_socket_delete( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc ) 
{
  ...
  CHECK_NUM_ARGS /* make sure all parameter are there */
	...
	if ( IS_SIGN || IS_ALL ) {
		/* Create ECC signing socket */
		rc = ecc_signing_socket_delete(myctx->ecc_info.signing_socket);
		if ( atlk_error(rc) ) {
			cli_print( cli, "ecc_signing_socket_create : %s\n", atlk_rc_to_str(rc));
			goto error;
		}
    ...
    rc = ecc_verification_socket_delete(myctx->ecc_info.verification_socket);
    if (atlk_error(rc)) {
...
int cli_v2x_ecc_sign_private_key( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  if ( argc == 0 ) {
    cli_print(cli, "Private key : " ECC_SCALAR_FMT, ECC_SCALAR_FMT_ARGS(myctx->ecc_info.signing_request.private_key) ); 
    return CLI_OK; 
	}
  ...
  hexstr_to_buffer(sign_key_str, strlen(sign_key_str), (uint8_t*) hex_arr, &msg_size);
  
  if ( msg_size != (PRIVATE_KEY_LEN * sizeof(int)) ) {
    cli_print ( cli, "ERROR : unable to convert key value, expected msg_size %d, received %d",(PRIVATE_KEY_LEN * sizeof(int)), msg_size);
    return CLI_ERROR;
  }

  int *pos = (int*) hex_arr; 
  for ( i=0; i < PRIVATE_KEY_LEN ; i++, pos++ ) {
    myctx->ecc_info.signing_request.private_key.value[i] = htonl(*pos);
  }
...
int cli_v2x_ecc_sign_request_send( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  /* Get Hash value */
  myctx->ecc_info.signing_request.request_id = request_id;
 
  msg_size = (size_t) (strlen(hash_key_str) / 2);
  hexstr_to_buffer(hash_key_str, strlen(hash_key_str), (uint8_t*) hex_arr, &msg_size);
  ...
  for ( i=0; i < HASH_DATA_LEN ; i++, pos++ ) {
		myctx->ecc_info.signing_request.digest.value[i] = htonl(*pos);
  }
  
  /* Send ECC signing request */
  rc = ecc_signing_request_send( myctx->ecc_info.signing_socket, &myctx->ecc_info.signing_request, NULL );
  if (atlk_error(rc)) {
...
{
  atlk_rc_t                 rc                = ATLK_OK;
  ecc_signing_response_t    signing_response  = ECC_SIGNING_RESPONSE_INIT;
  ...
  do {
    ...
    rc = ecc_signing_response_receive( myctx->ecc_info.signing_socket, &signing_response, NULL );
    if (atlk_error(rc)) {
  ...
  ecc_scalar_t s_scalar = signing_response.signature.s_scalar;
  ecc_scalar_t x_coordinate = signing_response.signature.R_point.x_coordinate;

  /* Print ECC signing response content */
  cli_print( cli, "ECC signature for request ID %llu:", signing_response.request_id);
  cli_print( cli, "r: " ECC_SCALAR_FMT "", ECC_SCALAR_FMT_ARGS(x_coordinate));
  cli_print( cli, "s: " ECC_SCALAR_FMT "", ECC_SCALAR_FMT_ARGS(s_scalar));
...
int cli_v2x_ecc_verification_set_public_key( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  if ( argc  == 0 ) {
    ...
    cli_print(cli, "x : " ECC_SCALAR_FMT, ECC_SCALAR_FMT_ARGS(myctx->ecc_info.verification_request.public_key.x_coordinate) ); 
    cli_print(cli, "y : " ECC_SCALAR_FMT, ECC_SCALAR_FMT_ARGS(myctx->ecc_info.verification_request.public_key.y_coordinate) ); 
    return CLI_OK; 
	}
  ...
  if ( strlen(verfication_key_x_str) > 0 ) {
    ...
    hexstr_to_buffer(verfication_key_x_str, strlen(verfication_key_x_str), (uint8_t*) hex_arr, &msg_size);
    ...
    pos = (int*) hex_arr; 
    for ( i=0; i < PRIVATE_KEY_LEN ; i++, pos++ ) {
      myctx->ecc_info.verification_request.public_key.x_coordinate.value[i] = *pos;
    }
  }
  ...
  if ( strlen(verfication_key_y_str) > 0 ) {
    ...
    hexstr_to_buffer(verfication_key_y_str, strlen(verfication_key_y_str), (uint8_t*) hex_arr, &msg_size);
    if ( msg_size != (PRIVATE_KEY_LEN * sizeof(int)) ) {
      cli_print ( cli, "ERROR : unable to convert key value" );
      return CLI_ERROR;
    }
    ...
    pos = (int*) hex_arr; 
    for ( i=0; i < PRIVATE_KEY_LEN ; i++, pos++ ) {
      myctx->ecc_info.verification_request.public_key.y_coordinate.value[i] = *pos;
    }
  }
  ...
  if ( strlen(point_type) > 0 ) {
    ...
    myctx->ecc_info.verification_request.public_key.point_type = ECC_POINT_UNCOMPRESSED;
  }
...
int cli_v2x_ecc_verification_set_signature( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  if ( argc  == 0 ) {
    ...
    cli_print(cli, "x : " ECC_SCALAR_FMT, ECC_SCALAR_FMT_ARGS(myctx->ecc_info.verification_request.signature.r_scalar) ); 
    cli_print(cli, "y : " ECC_SCALAR_FMT, ECC_SCALAR_FMT_ARGS(myctx->ecc_info.verification_request.signature.s_scalar) ); 
    return CLI_OK; 
	} 
  ...
  if ( strlen(signature_key_r_str) > 0 ) {
    ...
    hexstr_to_buffer(signature_key_r_str, strlen(signature_key_r_str), (uint8_t*) hex_arr, &msg_size);
    ...
    for ( i=0; i < SIGNATURE_KEY_LEN ; i++, pos++ ) {
      myctx->ecc_info.verification_request.signature.r_scalar.value[i] = *pos;
    }
  }
  ...
  if ( strlen(signature_key_s_str) > 0 ) {
    ...
    hexstr_to_buffer(signature_key_s_str, strlen(signature_key_s_str), (uint8_t*) hex_arr, &msg_size);
    if ( msg_size != (SIGNATURE_KEY_LEN * sizeof(int)) ) {
      cli_print ( cli, "ERROR : unable to convert key value" );
      return CLI_ERROR;
    }
...
int cli_v2x_ecc_verification_request_send( struct cli_def *cli, UNUSED(const char *command), char *argv[], int argc )  
{
  ...
  IS_HELP_ARG("ecc verification request -id N -hash 360c... -signuture  ");
  ...
  pos = (int*) hex_arr; 
  for ( i=0; i < HASH_DATA_LEN ; i++, pos++ ) {
		myctx->ecc_info.signing_request.digest.value[i] = *pos;
  }
